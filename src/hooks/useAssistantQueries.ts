...
export function useDeleteFileMutation(profileId: string, assistantName: string) {
  ...
  return useMutation({
    mutationFn: async (fileId: string) => {
      ...
      return { profileId, assistantName, fileId }
    },
    onMutate: async (fileId) => {
      ...
      // Snapshot the previous value
      const previousFiles = queryClient.getQueryData<AssistantFile[]>(
        assistantQueryKeys.files(profileId, assistantName)
      )

      // Optimistically update to show file as Deleting
      if (previousFiles) {
        queryClient.setQueryData(
          assistantQueryKeys.files(profileId, assistantName),
          previousFiles.map((file) =>
            file.id === fileId ? { ...file, status: 'Deleting' as const } : file
          )
        )
      }
    ...
    onSuccess: () => {
      // Invalidate files query to refetch without the deleted file
      queryClient.invalidateQueries({
        queryKey: assistantQueryKeys.files(profileId, assistantName),
      })
    },
...
// Describe File Query - fetches detailed file info including signedUrl
export function useFileDetailQuery(
  profileId: string | null,
  assistantName: string | null,
  fileId: string | null,
  enabled: boolean = true
  ...
  return useQuery({
    queryKey: [...assistantQueryKeys.files(profileId || '', assistantName || ''), 'detail', fileId || ''] as const,
