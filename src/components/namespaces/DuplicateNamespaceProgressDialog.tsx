...
interface DuplicateNamespaceProgressDialogProps {
  open: boolean
...
export function DuplicateNamespaceProgressDialog({
  open,
  onOpenChange,
  sourceNamespace,
  targetNamespace,
  progress,
  onCancel,
}: DuplicateNamespaceProgressDialogProps) {
  const displaySource = sourceNamespace || '(default)'
  ...
  const isWorking = progress.phase === 'copying'
        ...
        >
          <div className="px-5 pt-5 pb-4 text-center">
            <DialogPrimitive.Title className="text-[13px] font-semibold text-foreground">
              {isComplete ? 'Duplication Complete' : isError ? 'Duplication Failed' : isCancelled ? 'Duplication Cancelled' : 'Duplicating Namespace'}
            </DialogPrimitive.Title>
            <DialogPrimitive.Description className="mt-2 text-[11px] text-muted-foreground leading-[1.4]">
              {isComplete ? (
                <>Copied <span className="font-medium text-foreground">{progress.processedVectors.toLocaleString()}</span> vectors to <span className="font-medium text-foreground">{targetNamespace}</span></>
              ) : isError ? (
                <span className="text-destructive">{progress.message}</span>
              ) : isCancelled ? (
                <>Duplication was cancelled. <span className="font-medium text-foreground">{progress.processedVectors.toLocaleString()}</span> of <span className="font-medium text-foreground">{progress.totalVectors.toLocaleString()}</span> vectors were copied.</>
              ) : (
                <>Copying <span className="font-medium text-foreground">{displaySource}</span> to <span className="font-medium text-foreground">{targetNamespace}</span></>
              )}
